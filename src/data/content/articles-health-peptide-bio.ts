...
export const ARTICLES_HEALTH_PEPTIDE_BIO: ContentPageConfig[] = [
  // ============================================================
  // 1. TITRATION ESCALATION
  // ============================================================
  {
    id: 'art-pep-what-is-titration-escalation',
    kind: 'guide',
    section: 'guides',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    slug: 'what-is-titration-escalation',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Titration Escalation Calculates — Weekly Dose Step-Up | Kefiw',
    h1: 'What Titration Escalation Calculates',
    subhead: 'Linear weekly step-up schedule for GLP-1 titration — no guesswork, no drift.',
    outcomeLine: 'Titration Escalation answers one question: what dose am I supposed to draw this week?',
    description: 'Titration Escalation runs the linear step-up schedule for semaglutide and tirzepatide. Deterministic dose-by-week output, U-100 unit conversion, no drift.',
    keywords: ['GLP-1 titration schedule', 'semaglutide escalation', 'tirzepatide step-up', 'weekly peptide dose', 'titration calculator'],
    intro: 'Clinical titration is linear step-up, not vibes. Current concentration {{lm:conc}} feeds the unit math — if reconstitution drifts, so does every dose on the schedule.',
    keyPoints: [
      ...
      'Skip a step if tolerance fails — hold current dose an extra 4 weeks, do not drop back.',
    ...
    thresholds: {
      cyan: 'On-schedule dose, tolerance stable, concentration verified — execute.',
      gold: 'Step pending next week or mild GI — hold pattern, log symptoms.',
      magenta: 'Missed doses, concentration drift, or severe GI — recompute from last confirmed injection.',
    },
    formulaAnchor: {
    ...
    logicalGates: [
      'Confirm last injection date and dose. No guessing from memory.',
  ...
  // ============================================================
  {
    id: 'art-pep-what-is-peptide-inventory',
    kind: 'guide',
    section: 'guides',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    slug: 'what-is-peptide-inventory',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Peptide Inventory Calculates — Doses & Reorder Window | Kefiw',
    h1: 'What Peptide Inventory Calculates',
    subhead: 'Doses remaining, empty date, and reorder trigger — no running out mid-titration.',
    outcomeLine: 'Peptide Inventory answers one question: when does this vial stack go empty, and when must I reorder?',
    description: 'Peptide Inventory forecasts doses remaining, empty date, and reorder trigger based on vial mass, dose, interval, and vendor lead time. U-100 unit math baked in.',
    keywords: ['peptide vial tracking', 'GLP-1 reorder calculator', 'semaglutide inventory', 'peptide empty date', 'vendor lead time'],
    ...
    keyPoints: [
      ...
      'Count partial doses honestly — dead-volume residue below the stopper does not make the next dose.',
      'Stack multiple peptides? Compute each separately; the stack empty_date = min(empty_date) across peptides.',
    ],
    examples: [
      ...
      { title: '5 mg vial, 2.5 mg weekly', body: 'doses_remaining = 2. days_remaining = 14. Order the replacement before drawing the first dose — no buffer.' },
    ...
    logicalGates: [
      ...
      'Divide by dose_mg, floor — partial dose does not count.',
  ...
  // ============================================================
  {
    id: 'art-pep-what-is-vendor-roi',
    kind: 'guide',
    section: 'guides',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    slug: 'what-is-vendor-roi',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Vendor ROI Calculates — Cost Per Microgram & Dose | Kefiw',
    h1: 'What Vendor ROI Calculates',
    subhead: 'Cost per microgram and cost per dose across vendors — price discipline for peptide purchasing.',
    outcomeLine: 'Vendor ROI answers one question: which vendor gives me the lowest cost-per-dose after shipping?',
    description: 'Vendor ROI computes cost per microgram ((price + shipping) / mass_mcg) and cost per dose. Ranks vendors ascending. U-100 dose math ready.',
    keywords: ['peptide vendor cost comparison', 'cost per dose peptide', 'semaglutide vendor ranking', 'tirzepatide price per mg', 'peptide shipping cost math'],
    intro: 'Sticker price lies. Current waste {{lm:waste}} means every $ per mg is already discounted — the vendor with the cheapest mg may deliver the most expensive effective dose.',
    keyPoints: [
      ...
      'Rank vendors ascending by cost_per_dose. Shipping is not free — bake it in.',
      'Adjust for purity and peptide-content-factor: effective_mg = label_mg × purity × content_factor.',
      'U-100 dose conversion: dose_mcg = dose_mg × 1000; injection volume still follows 1 unit = 0.01 mL.',
      'Minimum-order discounts flatter cost-per-mg but bloat inventory. Run Peptide Inventory against the bulk empty_date before committing.',
    ],
    examples: [
    ...
    thresholds: {
      cyan: 'Cost-per-dose in bottom tercile across 3+ verified vendors — procure.',
    ...
    logicalGates: [
      ...
      'Multiply by dose_mcg for cost_per_dose.',
  ...
  // ============================================================
  {
    id: 'art-pep-what-is-travel-planner',
    kind: 'guide',
    section: 'guides',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    ...
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Travel Planner Calculates — Peptide Transit Load | Kefiw',
    h1: 'What Travel Planner Calculates',
    subhead: 'Doses, total mass, and vial count for a trip — per-peptide and stack-aggregate.',
    outcomeLine: 'Travel Planner answers one question: how many vials and how much mass do I pack for N days away?',
    description: 'Travel Planner computes doses, mass, and vial count per peptide and across the full stack for any trip length. U-100 volume math covers syringe packing.',
    keywords: ['peptide travel calculator', 'GLP-1 travel dose', 'peptide trip packing', 'semaglutide travel vials', 'peptide transit planner'],
    ...
    keyPoints: [
      'Formula per peptide: doses = ceil(trip_days / interval_days); total_mg = doses × dose_mg. Concentration {{lm:conc}} converts mg to mL for TSA liquid rules.',
      'Aggregate: stack_total_mg = sum(peptide_total_mg). Vial count = ceil(stack_total_mg / vial_mass_mg) per peptide, not summed.',
    ...
    examples: [
      { title: '10-day trip, semaglutide 0.5 mg weekly', body: 'doses = ceil(10/7) = 2. total_mg = 1 mg. At 2 mg/mL: 0.5 mL = 50 units across 2 injections. Single 5 mg vial covers it. Concentration {{lm:conc}}.' },
      { title: '21-day trip, stack: sema 1 mg/wk + BPC-157 250 mcg/day', body: 'Sema: doses=3, mass=3 mg. BPC: doses=21, mass=5.25 mg. Two vials minimum — one per peptide.' },
      { title: '45-day trip, tirzepatide 10 mg weekly', body: 'doses = ceil(45/7) = 7. total_mg = 70 mg. Across 10 mg vials: 7 vials. At 5 mg/mL: 14 mL total, 1400 units.' },
    ],
    whenToUse: [
    ...
    logicalGates: [
      ...
      'Per peptide: compute doses, mass, vial count separately.',
      'Sum vials across peptides — aggregate mass does not share vials.',
  ...
  // ============================================================
  // 5. PEPTIDE HALF-LIFE (HDS-1)
  // ============================================================
  {
    id: 'art-pep-what-is-peptide-half-life',
    kind: 'guide',
    section: 'guides',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    slug: 'what-is-peptide-half-life',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Peptide Half-Life Calculates — Exponential Decay & Steady State | Kefiw',
    h1: 'What Peptide Half-Life Calculates',
    subhead: 'Per-dose decay, stacked residuals, and steady-state plateau — deterministic PK math.',
    outcomeLine: 'Peptide Half-Life answers one question: how much active peptide is in circulation right now, given the injection history?',
    description: 'Peptide Half-Life runs N(t) = N0 × 0.5^(t/h) per dose, sums across prior injections, and resolves steady-state at ~4.3 half-lives. Client-side JS, 2-decimal precision.',
    keywords: ['peptide half-life calculator', 'semaglutide decay', 'tirzepatide steady state', 'peptide pharmacokinetics', 'exponential decay peptide'],
    intro: 'Single-dose math is easy; stacking is where intuition fails. Concentration {{lm:conc}} feeds the dose amount, but the residual curve is the real state variable.',
    keyPoints: [
      'Formula: N(t) = N₀ × 0.5^(t/h) per dose. Total circulating = sum across all prior doses. Concentration {{lm:conc}} sets N₀ per injection.',
      'Steady state reached at ~4.3 half-lives when interval ≤ half-life. Semaglutide (h ≈ 165 hr / 7 days): steady state ~4 weeks.',
      'If interval > half-life, no steady state — each dose peaks then decays alone. Short-half-life peptides (BPC-157 ~4 hr) act this way on daily protocols.',
      'Client-side JS; 2-decimal precision. No server round-trip.',
      'Missed dose impact: residual from prior doses covers ~1.5× the current interval before a true trough. Delay 2 weeks on semaglutide = real gap.',
    ],
    examples: [
      { title: 'Semaglutide week 4, 1 mg weekly, h = 7 days', body: 'N(t) per dose at t=7d = 0.50 mg. Dose 1 remnant at week 4 = 1 × 0.5^4 = 0.06 mg. Sum across 4 doses ≈ 1.88 mg circulating — approaching steady state.' },
      { title: 'BPC-157 daily 250 mcg, h = 4 hr', body: 'N(t) at t=24h = 250 × 0.5^6 ≈ 3.9 mcg residual. Steady state = 4.3 × 4h = 17 hr, well within daily cycle. Effectively single-dose behavior.' },
    ...
    thresholds: {
      ...
      gold: 'Approaching steady state (weeks 2-3) or missed one dose — recompute before next injection.',
      magenta: 'Interval > half-life or 2+ missed doses — residual insufficient, restart titration from last confirmed level.',
    },
    formulaAnchor: {
      caption: 'Deterministic Formula',
    ...
    logicalGates: [
      'Enter peptide half-life h (hours). Use 165h semaglutide, 120h tirzepatide, 4h BPC-157.',
      'Log every prior dose with timestamp — no approximations.',
      'Compute N(t) per dose, sum across dose history.',
      'Compare interval to h; if interval > h, single-dose behavior applies.',
  ...
  // ============================================================
  // 6. SARCOPENIA GUARD (SAR-1)
  // ============================================================
  {
    id: 'art-pep-what-is-sarcopenia-guard',
    kind: 'guide',
    section: 'guides',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    slug: 'what-is-sarcopenia-guard',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Sarcopenia Guard Calculates — Lean Mass Loss Ratio | Kefiw',
    h1: 'What Sarcopenia Guard Calculates',
    subhead: 'Lean-mass-to-total-loss ratio — catches muscle bleeding before the scale celebrates it.',
    outcomeLine: 'Sarcopenia Guard answers one question: of the weight I lost, how much was muscle I did not mean to lose?',
    description: 'Sarcopenia Guard computes lean_ratio_pct = (lean_loss / total_loss) × 100 from DEXA or bioimpedance deltas. Critical >= 25% — triggers ULH-01 Magenta flag.',
    keywords: ['sarcopenia GLP-1', 'lean mass loss peptide', 'muscle loss semaglutide', 'lean ratio calculator', 'tirzepatide muscle preservation'],
    ...
    keyPoints: [
      ...
      'Critical >= 25% — writes peptide_sarcopenia_critical flag → ULH-01 Magenta pulse.',
    ...
    faq: [
      ...
      { q: 'Can peptides like BPC-157 or retatrutide change the ratio?', a: 'Retatrutide trial data suggests better lean preservation than semaglutide; BPC-157 has no direct body-composition evidence. Do not assume protection without measurement.' },
    ],
    thresholds: {
    ...
    logicalGates: [
      ...
      'If ratio >= 25, write peptide_sarcopenia_critical flag — ULH-01 pulses Magenta.',
  ...
  // ============================================================
  {
    id: 'art-pep-what-is-weight-trajectory',
    kind: 'guide',
    section: 'guides',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    slug: 'what-is-weight-trajectory',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Weight Trajectory Calculates — STEP & SURMOUNT Curves | Kefiw',
    h1: 'What Weight Trajectory Calculates',
    subhead: 'Your actual weight-loss curve vs STEP (semaglutide) and SURMOUNT (tirzepatide) clinical trial baselines.',
    outcomeLine: 'Weight Trajectory answers one question: am I tracking clinical expectation, ahead of it, or falling behind?',
    description: 'Weight Trajectory linearly interpolates STEP (sema 15% by w68) and SURMOUNT (tirz 20% by w72) curves. Delta_pct = actual − expected. Flags lag and runaway loss.',
  ...
  // ============================================================
  // 8. PEPTIDE DEGRADATION (DEG-1)
  // ============================================================
  {
    id: 'art-pep-what-is-peptide-degradation',
    kind: 'guide',
    section: 'guides',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    slug: 'what-is-peptide-degradation',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Peptide Degradation Calculates — Arrhenius Thermal Decay | Kefiw',
    h1: 'What Peptide Degradation Calculates',
    subhead: 'Retained potency after thermal exposure — Arrhenius-style decay, lyophilized vs reconstituted rates.',
    outcomeLine: 'Peptide Degradation answers one question: after N hours at temperature X, how much active peptide survives?',
    description: 'Peptide Degradation runs simplified Arrhenius potency_pct = 100 × exp(−k × hours). Per-temperature rate constants. Reconstituted decays 10× faster. Critical < 85% potency.',
    keywords: ['peptide degradation calculator', 'semaglutide thermal decay', 'peptide storage potency', 'reconstituted peptide shelf life', 'Arrhenius peptide'],
    intro: 'A vial above fridge temperature is losing potency every hour. Current retained potency {{lm:potency}} — below 85% means every downstream calc (inventory, titration, vendor ROI) is wrong by the same margin.',
    keyPoints: [
      ...
      'Critical < 85% potency — writes peptide_degradation_critical flag → ULH-01 Magenta pulse.',
    ...
    faq: [
      ...
      { q: 'Does freezing protect better than fridge?', a: 'For lyophilized yes; for reconstituted no — freeze-thaw cycles aggregate peptide chains and destroy more potency than steady fridge storage. Never freeze reconstituted vials.' },
    ],
    thresholds: {
