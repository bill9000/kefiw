...
export const ARTICLES_HEALTH_PEPTIDE_PREP: ContentPageConfig[] = [
  // ============================================================
  // PRC-1 — PEPTIDE RECONSTITUTION
  // ============================================================
  {
    id: 'art-pep-what-is-peptide-recon',
    kind: 'guide',
    section: 'guides',
    slug: 'what-is-peptide-recon',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Peptide Reconstitution Calculates — mg/mL Concentration | Kefiw',
    h1: 'What Peptide Reconstitution Calculates',
    subhead: 'The working concentration of a lyophilized vial after bacteriostatic water is added.',
    outcomeLine: 'PRC-1 answers one question: after reconstitution, how many mg of peptide sit in every mL — and every U-100 unit?',
    description: 'PRC-1 reconstitutes a lyophilized vial: concentration = vial mass / BAC water volume. Converts to U-100 units per mg for insulin-syringe dosing.',
    keywords: ['peptide reconstitution calculator', 'bac water ratio', 'mg per ml peptide', 'u-100 units per mg', 'lyophilized vial math'],
    intro: 'A vial labeled "5 mg" is not a dose — it is a mass sitting dry. Add water and the concentration depends entirely on how much. Current working concentration: {{lm:concentration}}. Every downstream calculation (units, mcg, dose volume) fails if this number is wrong.',
    keyPoints: [
      ...
      'BAC water volume is the operator choice, not the vial label. 5 mg peptide + 2 mL water = 2.5 mg/mL; same vial + 5 mL water = 1 mg/mL.',
      'Precision matters to 2 decimals. 2.50 mg/mL vs 2.47 mg/mL is a 1.2% dosing error that compounds across a vial.',
      'Pipe this concentration to GLP-1 Units (DSE-1) and MCG-per-Unit (CNV-1). Do not re-derive downstream.',
    ],
    examples: [
      { title: '5 mg semaglutide + 2 mL BAC', body: 'concentration = 5 / 2 = 2.50 mg/mL. units_per_mg = 100 / 2.50 = 40 units/mg. A 0.25 mg dose = 10 units on a U-100 syringe.' },
      { title: '10 mg retatrutide + 4 mL BAC', body: 'concentration = 10 / 4 = 2.50 mg/mL. Same ratio, larger total volume — 40 doses of 0.25 mg instead of 20.' },
    ...
    thresholds: {
      cyan: 'Concentration matches operator target within 2% — dosing pipeline clean.',
      gold: 'Concentration within 5% of target — acceptable but re-verify BAC volume.',
      magenta: 'Concentration off by >5% or undefined — do not dose; reconstitute again.',
    },
    formulaAnchor: {
  ...
  // ============================================================
  {
    id: 'art-pep-what-is-peptide-stack',
    kind: 'guide',
    section: 'guides',
    slug: 'what-is-peptide-stack',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Peptide Stack Calculates — Two-Compound Shared Vial | Kefiw',
    h1: 'What Peptide Stack Calculates',
    subhead: 'Combined concentration when two peptides share one BAC-water volume.',
    outcomeLine: 'STK-1 answers one question: for two peptides in one vial, what mg of each lands in every draw?',
    description: 'STK-1 computes concentration for two peptides reconstituted in a shared vial. Each draw delivers proportional mass of A and B based on individual mg/mL.',
    keywords: ['peptide stack calculator', 'dual peptide reconstitution', 'combined peptide vial', 'co-reconstitution math', 'stack concentration'],
    intro: 'Stacking two peptides in one vial halves your injection count but doubles the math surface. Current working concentration: {{lm:concentration}}. One draw now delivers two mass payloads; if either concentration is wrong, both doses are wrong.',
    keyPoints: [
      ...
      'Dead space in syringe wastes both peptides simultaneously. Log loss per draw on both sides.',
    ],
    examples: [
    ...
    faq: [
      ...
      { q: 'Does co-reconstitution affect peptide stability?', a: 'Some pairs degrade faster together (e.g. aggregation-prone compounds). The math is unaffected; shelf-life is. Treat stacked vials as shorter-lived.' },
    ],
    thresholds: {
  ...
  // ============================================================
  {
    id: 'art-pep-what-is-salt-correction',
    kind: 'guide',
    section: 'guides',
    ...
    subhead: 'Actual peptide mass after stripping counter-ion weight from the labeled vial mass.',
    outcomeLine: 'SCF-1 answers one question: of the "5 mg" on the label, how many mg are peptide and how many are salt?',
    description: 'SCF-1 applies the peptide content percentage to labeled mass. TFA salts typically 76%, acetate 89%. Actual mass drives concentration, not label.',
    keywords: ['tfa salt correction', 'acetate salt peptide', 'net peptide mass', 'peptide content percentage', 'salt-adjusted dose'],
    intro: 'Labeled vial mass is total weight — peptide plus counter-ion. TFA salt forms drop real peptide content to ~76%; acetate to ~89%. Current working concentration: {{lm:concentration}}. Skip this correction and every downstream dose is silently under-delivered.',
    keyPoints: [
      'Formula: actual_mass_mg = label_mass_mg × (peptide_content_pct / 100).',
      'TFA (trifluoroacetate) salt: typical content 76% — label 5 mg → actual 3.80 mg peptide.',
      'Acetate salt: typical content 89% — label 5 mg → actual 4.45 mg peptide.',
      'Free base (no salt): 100% — rare outside research-grade material. Verify on COA.',
      'Corrected mass feeds PRC-1 reconstitution. Using label mass overstates concentration and underdoses the patient.',
    ],
    examples: [
      { title: '5 mg label, TFA salt (76%)', body: 'actual_mass = 5 × 0.76 = 3.80 mg. Reconstituted in 2 mL → 1.90 mg/mL (not 2.50). 0.25 mg "dose" is actually 0.19 mg.' },
      { title: '10 mg label, acetate salt (89%)', body: 'actual_mass = 10 × 0.89 = 8.90 mg. In 4 mL → 2.23 mg/mL. An 11% dosing gap if uncorrected.' },
    ...
    thresholds: {
      cyan: 'Peptide content ≥ 85% and matches COA — correction applied, ready for PRC-1.',
      gold: 'Content 70–85% — common TFA range, verify COA before reconstituting.',
      magenta: 'Content < 70% or unknown — do not dose; return vial or demand updated COA.',
    },
    formulaAnchor: {
      caption: 'Deterministic Formula',
    ...
    logicalGates: [
      'Read COA for salt form and peptide content percentage.',
  ...
  // ============================================================
  {
    id: 'art-pep-what-is-solubility-limit',
    kind: 'guide',
    section: 'guides',
    ...
    subhead: 'Whether the planned concentration stays below the peptide\'s BAC-water solubility maximum.',
    outcomeLine: 'SOL-1 answers one question: will this peptide fully dissolve in this volume, or sludge at the bottom of the vial?',
    description: 'SOL-1 compares planned concentration against peptide-specific solubility maximum (BPC-157 ~10, Semaglutide ~20 mg/mL). Sludge means uneven dosing.',
    keywords: ['peptide solubility limit', 'bac water solubility', 'peptide sludge', 'semaglutide solubility', 'bpc-157 dissolution'],
    intro: 'Peptides dissolve up to a hard maximum — beyond that, solid sits in the vial and draws deliver unpredictable mass. Current working concentration: {{lm:concentration}}. SOL-1 catches the problem before reconstitution, not after you draw a clump.',
    keyPoints: [
      'Formula: concentration_mg_ml = mass / volume. Compare to peptide-specific maximum.',
      'Typical maxima: BPC-157 ~10 mg/mL, TB-500 ~10, Semaglutide ~20, Retatrutide ~20, Tirzepatide ~20.',
      'If concentration > limit, peptide partially precipitates. Draws become lottery tickets on mass.',
    ...
    examples: [
      { title: '10 mg BPC-157 / 0.5 mL BAC', body: 'conc = 20 mg/mL vs limit ~10. Sludge risk HIGH. Double BAC to 1 mL → 10 mg/mL, right at limit.' },
      { title: '10 mg Semaglutide / 1 mL BAC', body: 'conc = 10 mg/mL vs limit ~20. Well within margin. Full dissolution expected at room temp.' },
      { title: '20 mg Retatrutide / 2 mL BAC', body: 'conc = 10 mg/mL vs limit ~20. Dissolves clean. Headroom available for a stacked compound if compatible.' },
    ],
    whenToUse: [
    ...
    faq: [
      ...
      { q: 'Can I add more BAC later if it sludges?', a: 'Yes — dilution works. Recompute concentration in PRC-1 and relabel the vial. Do not dose until fully dissolved.' },
    ],
    thresholds: {
    ...
    logicalGates: [
      'Look up peptide solubility maximum on vendor COA.',
      'Compute planned concentration = mass / BAC volume.',
      'Compare concentration to limit. Add BAC if concentration > limit.',
      'For stacks, sum both peptide concentrations against the weaker compound\'s limit.',
  ...
  // ============================================================
  // DSE-1 — GLP UNITS / DOSE-TO-UNITS
  // ============================================================
  {
    id: 'art-pep-what-is-glp-units',
    kind: 'guide',
    section: 'guides',
    slug: 'what-is-glp-units',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What GLP Units Calculates — Dose mg to U-100 Units | Kefiw',
    h1: 'What GLP Units Calculates',
    subhead: 'The U-100 insulin-syringe unit count that delivers a target mg dose at a given concentration.',
    outcomeLine: 'DSE-1 answers one question: how many units on the U-100 syringe deliver this mg dose at this concentration?',
    description: 'DSE-1 converts a mg dose to U-100 units via volume_ml = dose_mg / concentration_mg_ml, then units = volume × 100. Verifies against syringe capacity.',
    keywords: ['glp-1 dose units', 'u-100 insulin syringe conversion', 'mg to units peptide', 'semaglutide unit calculator', 'tirzepatide dosing'],
    intro: 'Peptide doses are prescribed in mg but drawn in units. A U-100 syringe holds 100 units in 1 mL — period. Current working concentration: {{lm:conc}}. Get the conversion wrong and you either cap out the syringe mid-draw or deliver a fractional dose.',
    keyPoints: [
      'Formula: volume_ml = dose_mg / concentration_mg_ml, then units = volume_ml × 100.',
      'U-100 syringe capacity = 100 units = 1 mL. If computed units > 100, dose cannot fit in one draw.',
      ...
      'If dose exceeds syringe, your concentration is too low — reconstitute stronger, do not split draws.',
    ],
    examples: [
    ...
    faq: [
      ...
      { q: 'Why does the tool flag "exceeds syringe"?', a: 'Because splitting doses across two draws doubles dead-space loss and multiplies measurement error. Reconstituting stronger is the correct fix.' },
    ],
    thresholds: {
      cyan: 'Dose lands at 5–80 units — clean draw, low error.',
      gold: 'Dose lands at 80–100 units or below 5 — near syringe limits or fractional-unit territory.',
      magenta: 'Dose exceeds 100 units — reconstitute stronger; split-draw is not an option.',
    },
    formulaAnchor: {
      caption: 'Deterministic Formula',
  ...
  // ============================================================
  {
    id: 'art-pep-what-is-mcg-per-unit',
    kind: 'guide',
    section: 'guides',
    slug: 'what-is-mcg-per-unit',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What MCG Per Unit Calculates — Concentration to Micrograms | Kefiw',
    h1: 'What MCG Per Unit Calculates',
    subhead: 'The micrograms of peptide delivered by a single U-100 insulin-syringe unit.',
    outcomeLine: 'CNV-1 answers one question: at this concentration, how many mcg of peptide are in each U-100 unit?',
    ...
    keywords: ['mcg per unit peptide', 'u-100 mcg conversion', 'peptide dose in micrograms', 'insulin syringe mcg', 'bpc-157 units'],
    intro: 'GLP-1 dosing thinks in mg; growth-hormone and healing peptides think in mcg. Same U-100 syringe, different mental unit. Current working concentration: {{lm:concentration}}. CNV-1 converts once and locks the operator into a consistent scale.',
    keyPoints: [
      ...
      'Typical use: BPC-157, TB-500, CJC-1295, Ipamorelin, Tesamorelin — dosed in 100–500 mcg ranges.',
      'Pipe output back to DSE-1 as a sanity check: target_mcg ÷ mcg_per_unit should match DSE-1 units output.',
    ],
    examples: [
      { title: 'Concentration 1.00 mg/mL', body: 'mcg_per_unit = 1.00 × 10 = 10 mcg/unit. 250 mcg dose = 25 units. Clean decimal.' },
      { title: 'Concentration 2.50 mg/mL', body: 'mcg_per_unit = 2.50 × 10 = 25 mcg/unit. 500 mcg dose = 20 units. Low draw volume, minimal waste.' },
    ...
    faq: [
      ...
      { q: 'Should I think in mcg or units when dosing?', a: 'Whichever the peptide is prescribed in. CNV-1 lets you translate in either direction without recomputing from concentration each time.' },
    ],
    thresholds: {
      cyan: 'mcg_per_unit lands 5–50 — typical BPC/TB/healing range, clean dosing.',
  ...
  // ============================================================
  // SDS-1 — SYRINGE WASTE / DEAD SPACE
  // ============================================================
  {
    id: 'art-pep-what-is-syringe-waste',
    kind: 'guide',
    section: 'guides',
    slug: 'what-is-syringe-waste',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Syringe Waste Calculates — Dead Space Leakage | Kefiw',
    h1: 'What Syringe Waste Calculates',
    subhead: 'The peptide mass lost per draw to syringe dead space and the annualized dollar cost.',
    outcomeLine: 'SDS-1 answers one question: how much peptide evaporates into dead space per draw, and what does it cost per year?',
    ...
    keywords: ['syringe dead space waste', 'peptide leakage cost', 'insulin syringe hub waste', 'peptide vial economics', 'annual peptide cost'],
    intro: 'Every syringe has dead space — the volume in the hub and needle that never enters the body. Across a year of daily doses that volume compounds into a second vial of lost peptide. Current annual waste: {{lm:waste}}. SDS-1 prices that leak.',
    keyPoints: [
      ...
      'Low dead-space (LDS) insulin syringes cut hub loss by 60–80% vs standard. Measurable ROI at any daily dosing.',
  ...
  // ============================================================
  // MPF-1 — MASS PURITY FILTER
  // ============================================================
  {
    id: 'art-pep-what-is-mass-purity-filter',
    kind: 'guide',
    section: 'guides',
    slug: 'what-is-mass-purity-filter',
    clusterId: 'peptide-protocol',
    pageType: 'support',
    guideCategory: 'Peptide Protocol',
    toneProfile: 'sop',
    title: 'What Mass Purity Filter Calculates — HPLC-Adjusted Dose | Kefiw',
    h1: 'What Mass Purity Filter Calculates',
    subhead: 'Effective peptide mass after discounting for HPLC purity percentage on the COA.',
    outcomeLine: 'MPF-1 answers one question: given HPLC purity on the COA, what fraction of the label mass is actually active peptide?',
    ...
    keywords: ['peptide hplc purity', 'mass purity filter', 'effective peptide dose', 'coa purity adjustment', 'peptide label accuracy'],
    intro: 'HPLC purity on a COA is not marketing — it is the fraction of label mass that is the target molecule. Current working concentration: {{lm:concentration}}. A 95%-pure vial delivers 95% of the dose you think; MPF-1 pushes that correction back into PRC-1 before it hits the patient.',
    keyPoints: [
      'Formula: effective_dose_mg = label_dose_mg × (purity_pct / 100).',
      'Research-grade peptides commonly list 97–99% HPLC. Under 95% should trigger re-verification or return.',
    ...
    faq: [
      ...
      { q: 'How does this affect SDS-1 waste math?', a: 'Effective mass lowers the true per-mg cost but raises the % of active peptide lost to dead space. SDS-1 uses post-filter concentration automatically if PRC-1 was updated.' },
    ],
    thresholds: {
      cyan: 'HPLC ≥ 97% — effective dose close to label; minor correction.',
      gold: 'HPLC 90–97% — apply filter, log discrepancy, verify COA on next batch.',
      magenta: 'HPLC < 90% or absent — do not dose; return vial or flag vendor.',
    },
    formulaAnchor: {
      caption: 'Deterministic Formula',
