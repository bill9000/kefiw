...
3. **Wire** it into the article config — add `ogImage: '/og/{slug}.jpg'` to the corresponding `ContentPageConfig` in `src/data/content/*.ts`. The `GuideLayout` forwards this to `og:image` and `twitter:image` automatically; no other wiring needed.
